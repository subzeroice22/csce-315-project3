...
class gameStart {
	...
	boolean PlayerIsAI(Square Player) {
		if (Player == Square.player1) {
			return (Global2.AIlevelP1 != "OFF");
		} else if (Player == Square.player2) {
			return (Global2.AIlevelP2 != "OFF");
		} else {
	...
	tempValid.SetBoard(Global2.game.GetBoard());
	Vector< Pair > vals = tempValid.GetValidMoves(Global2.CurrentPlayer);
	int randomMove = 0 + (int)Math.random() * vals.size();
	Global2.coordinate.Set(randomMove, randomMove);
	...
	int api(String commandLine,int client){
		...
		while(true) {
			...
			if(PlayerIsAI(Square.player1)) {
				System.out.print("AI	:" + Square.player1 + ": BLACK\n");
			} else {
				System.out.print("Human	:" + Square.player1 + ": BLACK\n");
			}
			
			System.out.print("Player2: ");
			if(PlayerIsAI(Square.player2)) {
				System.out.print("AI	:" + Square.player2 + ": WHITE\n");
			} else {
				System.out.print("Human :" + Square.player2 + ": WHITE\n");
			}
	...
	int endGameEvaluator(GameReversi childBoard){
		Square opponent;
		if	(childBoard.Count(Square.empty) == 0||//No more empty squares or niether player has a move, end game 
